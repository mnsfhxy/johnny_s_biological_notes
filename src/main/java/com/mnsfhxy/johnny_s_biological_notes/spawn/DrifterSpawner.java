...
public class DrifterSpawner implements CustomSpawner {
    ...
    private final int SPAWN_DELAY = UtilLevel.TIME.MINUTE.getTick()*20;
    private final int TICK_DELAY = UtilLevel.TIME.MINUTE.getTick();
    ...
    public int tick(ServerLevel pLevel, boolean pSpawnHostiles, boolean pSpawnPassives) {
        if (--this.tickDelay > 0) {
            ...
            if(pLevel.getPlayers(LivingEntity::isAlive).size()>0){
                this.tickDelay = TICK_DELAY;
                this.spawnDelay -= TICK_DELAY;
            }
...
//                }
    ...
    private BlockPos findSpawnPositionNear(LevelReader pLevel, BlockPos pPos, int pMaxDistance) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (pLevel.getBlockState(pPos.below()).is(Blocks.GRASS_BLOCK)||NaturalSpawner.isSpawnPositionOk(SpawnPlacements.Type.ON_GROUND, pLevel, blockpos1, EntityType.WANDERING_TRADER)) {
                blockpos = blockpos1;
                break;
            }
        }
